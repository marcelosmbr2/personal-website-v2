...
import { IconCloudDownload, IconDatabase, IconFolder, IconTrash } from '@tabler/icons-react';
...
import { index as backupIndex, destroy, download, runDatabase, runStorage } from '@/routes/admin/backup';
    ...
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    <Card className="shadow-none">
                        <CardHeader>
                            ...
                            <CardDescription>Gera um dump do banco de dados em formato ZIP.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Form action={runDatabase()}>
                                {({ processing }) => (
                                    <Button disabled={processing}>
                                        {processing ? 'Executando...' : 'Executar Backup'}
                                    </Button>
                                )}
                            </Form>
                        </CardContent>
                    ...
                    <Card className="shadow-none">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Compacta os arquivos do storage público (uploads, avatares, CVs).
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                ...
                <div>
                    <h2 className="mb-3 text-base font-medium">Backups existentes</h2>
                    {backups.length === 0 ? (
                                        ...
                                        <TableCell>
                                            <div className="flex gap-1">
                                                <Button variant="ghost" size="icon" asChild>
                                                    <a href={download.url({ file: backup.name })}>
            ...
            <Dialog open={!!deleteBackup} onOpenChange={() => setDeleteBackup(null)}>
                <DialogContent>
                    ...
                    <DialogDescription>
                        ...
                        <span className="font-medium text-foreground">{deleteBackup?.name}</span>? Essa ação não pode
                        ser desfeita.
                    </DialogDescription>
                    {deleteBackup && (
                        <Form action={destroy({ file: deleteBackup.name })} onSuccess={() => setDeleteBackup(null)}>
                            {({ processing }) => (
                                <DialogFooter>
                                    <DialogClose asChild>
